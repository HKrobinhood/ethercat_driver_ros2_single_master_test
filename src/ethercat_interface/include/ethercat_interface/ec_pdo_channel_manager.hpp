...
void ec_update(uint8_t * domain_address)
{
  ...
  if (pdo_type == TPDO) {
    ...
    if (interface_index >= 0) {
      ...
      // std::cout << "[TPDO] Read from channel " << std::showbase << std::hex << index << std::dec
      //           << " last_value: " << last_value
      //           << " | stored to state_interface[" << interface_index << "]" << std::endl;
    }
    ...
    if (interface_index >= 0 &&
      ...
      !override_command)
    {
      double cmd_value = command_interface_ptr_->at(interface_index);
      double write_value = factor * cmd_value + offset;

      ec_write(domain_address, write_value);
      // std::cout << "[第1段] RPDO write to channel " << std::showbase << std::hex << index << std::dec
      //           << " cmd_value: " << cmd_value
      //           << " | factor: " << factor
      //           << " | offset: " << offset
      //           << " | computed write_value: " << write_value << std::endl;
    } else {
      if (!std::isnan(default_value)) {
        ec_write(domain_address, default_value);
        // std::cout << "[第2段] RPDO write to channel " << std::showbase << std::hex << index << std::dec
